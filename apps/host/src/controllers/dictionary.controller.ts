...
import { ChangeWordsIndexDto, CreateWordDto, CreateWordsDto, DeleteWordDto, DeleteWordsDto, GetWordsDto } from '@app/shared/types/dto/dictionaryService.dto';
...
export interface DictionaryService {
    ...
    getWords: (args: GetWordsDto, metadata: any) => Observable<GetDictionaryWordsResType>;
    deleteWord: (args: DeleteWordDto, metadata: any) => any;
    deleteWords: (args: DeleteWordsDto, metadata: any) => any;
    changeWordsIndex: (args: ChangeWordsIndexDto, metadata: any) => any;
}
...
export class DictionaryController implements OnModuleInit {
    ...
    @Post('/words/changeIndex')
    async changeWordsIndex(@Body() dto: ChangeWordsIndexDto, @Res() res: Response, @Cookies('Authorization') token) {
        try {
            const metadata = new Metadata();
            metadata.add('cookie', `Authorization=${token}`);

            const response = await firstValueFrom(this.dictionaryService.changeWordsIndex(dto, metadata));
            return res.send(response)
        } catch (e) {
            console.log(e);
            const error = objectError(e.details)
